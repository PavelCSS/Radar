...
var start=function(a){timer||($el=$(a.target),timer=window.setTimeout(function(){$el.addClass("touch")},50))},stop=function(a){timer&&(window.clearTimeout(timer),timer=null,$el=$(a.target),setTimeout(function(){$el.removeClass("touch")},10))};$("body").on(events[0],start);$("body").on(events[1],stop);$(document).on("tap",".menu-btn",function(a){event.preventDefault();showMenu($(this))}).on("tap",".dropdown-btn",function(){event.preventDefault();showDropdown($(this))}).on("tap",".sub-menu-btn",function(){event.preventDefault();$(this).toggleClass("active").nextAll("ul").toggleClass("open")}).on("touchmove",".menu-btn.active, .dropdown-btn.active",function(a){$(".dropdown-menu").removeClass("open");$("body").removeAttr("data-menu-open");$(".menu-btn, .dropdown-btn").removeClass("active")});
...
function failUpload(a){hideLoading();alert("Please try again...");console.log("An error has occurred: Code = "+a.code)};var backBtnSwitch=!0;function goBack(){"home"!==window.location.hash.replace("#","")?window.history.back():(backBtnSwitch=!0,document.removeEventListener("backbutton",goBack,!1))}
function openPage(){var a=window.location.hash.replace("#",""),a=a||"true"!==localStorage.getItem("login")?a:"home";backBtnSwitch&&"home"!==a&&(backBtnSwitch=!1,document.addEventListener("backbutton",goBack,!1));switchMenu=!1;"login"===a&&(localStorage.removeItem("pageList"),localStorage.removeItem("login"));var b=localStorage.getItem("pageList"),b=b?JSON.parse(b):!1;if(a&&"undefined"!==typeof b[a])return goPage(a),!0;if("true"!==localStorage.getItem("login"))a="login";else if("undefined"===typeof pagesList[a])return!1;
pagesList[a]()}function showLoading(){$("html").addClass("loading")}function hideLoading(){$("html").removeClass("loading")};
